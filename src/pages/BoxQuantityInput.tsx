...
export const BoxQuantityInput: React.FC<BoxQuantityInputProps> = ({ navigateTo }) => {
  ...
  // 注文データを取得する関数
  const fetchOrderData = async (date: string, deptId: string, impId: string) => {
    try {
      // GSIを使用して効率的に注文データを取得
...
// 処理済み注文を取得する関数
const fetchDoneOrders = async (date: string, deptId: string, impId: string) => {
  try {
    // GSIを使用して効率的に処理済み注文を取得
    const doneOrdersResponse = await dataClient.models.Order.listOrdersByDeptAndImport({
      date: date,
      departmentIdImportId: {
        eq: {
          departmentId: deptId,
      ...
      filter: {  // 第2引数として渡す
        status: { eq: StatusTemplate.DONE }
      }
    });
    
    return doneOrdersResponse.data;
  } catch (error) {
    console.error('処理済み注文データ取得エラー:', error);
    throw error;
  }
};

  // 箱データを取得する関数
  const fetchBoxData = async (date: string, deptId: string) => {
    try {
      ...
      console.error('箱データ取得エラー:', error);
      throw error;
    }
  };

  // 注文ステータスを更新する関数
  const updateOrderStatus = async (orders: any[], status: OrderStatus) => {
    const updatePromises = orders.map(async order => {
      try {
        return await dataClient.models.Order.update({
        ...
        console.error(`注文ステータス更新エラー: ${order.storeId}`, error);
        throw error;
      }
    });
  ...
  // ImportWorkStatusを更新する関数
  const updateImportWorkStatus = async (date: string, deptId: string, impId: string, phase: string, progress?: string) => {
    try {
      ...
      return await dataClient.models.ImportWorkStatus.update(updateData);
    } catch (error) {
      console.error('ImportWorkStatus更新エラー:', error);
      throw error;
    }
  };
  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      try {
        ...
        const ordersData = await fetchOrderData(date, departmentId, importResult.importId);
        console.log('オーダーレスポンス', ordersData);
        
        // 処理済み注文を取得
        const doneOrders = await fetchDoneOrders(date, departmentId, importResult.importId);
        console.log('処理済み注文データ取得結果:', {
          totalDoneOrders: doneOrders.length,
    ...
    // sortingPhase または 中之島の有無 に基づいて currentRegion を決定
      if (importResult.sortingPhase === 'COMPLETED_NAKANOSHIMA' || isNakanoshimaMissing) {
        ...
        // 箱データを取得
        const boxData = await fetchBoxData(date, departmentId);
        
        // BoxData の型と実際のデータ構造の違いを解消するためにマッピング
        const mappedBoxData = boxData.map(box => ({
          date: box.date,
          storeId: box.storeId,
          ...
          importId: importResult.importId // importIdを保持
        }));
  ...
  async function navigateToNextStore() {
    ...
    try {
      ...
      const boxData = {
        ...
        importId: importId // importIdを直接保存
      };
      ...
      if (newlyCompleted.length === nakanoshimaStores.length) {
        try {
          ...
          await updateImportWorkStatus(
            currentDate, 
            departmentId as string, 
            importId, 
            'COMPLETED_NAKANOSHIMA'
          );
          ...
          // 最新の注文データを再取得（GSIを使用）
          const latestOrders = await fetchOrderData(currentDate, departmentId as string, importId);
  ...
  return;
      ...
      if (!nextStore) {
        try {
          ...
          // 注文ステータスを一括更新
          await updateOrderStatus(joetsuOrders, 'DONE');
